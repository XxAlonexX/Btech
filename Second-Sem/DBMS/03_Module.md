...
|--------------------|-----------------------------------------------------------------------------|-----------------------------------------------------------------------------|------------------------------------------------------------------------|
| Inner Join         | Returns rows that have matching values in both tables.                     | `SELECT A.name, B.suit FROM superheroes A INNER JOIN suits B ON A.id = B.superhero_id;` | Iron Man (Iron Suit), Spider-Man (Spider Suit), Wonder Woman (Golden Armor) |
| Left Outer Join    | Returns all rows from the left table, and the matched rows from the right table. If no match is found, NULL values are used. | `SELECT A.name, B.suit FROM superheroes A LEFT JOIN suits B ON A.id = B.superhero_id;` | Iron Man (Iron Suit), Spider-Man (Spider Suit), Wonder Woman (Golden Armor), Captain America (NULL) |
| Right Outer Join   | Returns all rows from the right table, and the matched rows from the left table. If no match is found, NULL values are used. | `SELECT A.name, B.suit FROM superheroes A RIGHT JOIN suits B ON A.id = B.superhero_id;` | Iron Man (Iron Suit), Spider-Man (Spider Suit), Wonder Woman (Golden Armor), NULL (Invisible Suit) |
| Full Outer Join    | Returns all rows when there is a match in either left or right table. If no match is found, NULL values are used. | `SELECT A.name, B.suit FROM superheroes A FULL OUTER JOIN suits B ON A.id = B.superhero_id;` | Iron Man (Iron Suit), Spider-Man (Spider Suit), Wonder Woman (Golden Armor), Captain America (NULL), NULL (Invisible Suit) |
| Cross Join         | Returns the Cartesian product of the two tables, i.e., every row from the left table is combined with every row from the right table. | `SELECT A.name, B.suit FROM superheroes A CROSS JOIN suits B;` | All possible combinations of superheroes and suits |
| Natural Join       | Automatically joins tables based on columns with the same name.            | `SELECT A.name, B.suit FROM superheroes A NATURAL JOIN suits B;` | Iron Man (Iron Suit), Spider-Man (Spider Suit), Wonder Woman (Golden Armor) |
| Self Join          | Joins a table to itself. Useful for hierarchical data.                     | `SELECT A.name AS Employee, B.name AS Manager FROM employees A JOIN employees B ON A.manager_id = B.id;` | Employee-Manager pairs |
| Theta Join         | Joins tables based on a condition other than equality.                      | `SELECT A.name, B.suit FROM superheroes A JOIN suits B ON A.id > B.superhero_id;` | Rows where the superhero ID is greater than the suit's superhero ID |
| Equi Join          | Joins tables based on an equality condition.                                | `SELECT A.name, B.suit FROM superheroes A JOIN suits B ON A.id = B.superhero_id;` | Iron Man (Iron Suit), Spider-Man (Spider Suit), Wonder Woman (Golden Armor) |
| Anti Join          | Returns rows from the left table that do not have a match in the right table. | `SELECT A.name FROM superheroes A LEFT JOIN suits B ON A.id = B.superhero_id WHERE B.superhero_id IS NULL;` | Captain America |
```sql
